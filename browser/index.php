...
//Application Title
define('APP_TITLE', 'Blog File Manager');
...
$root_path = '/home/ftp-goizper/ftp';
...
$directories_users = array(
    'blog' => '/home/ftp-goizper/ftp/uploads/'
);
...
// Backup Folder. To save all uploaded or created files.
$bakPath = '/var/www/files-bak/';
...
$dateFormat = "d\/m\/Y:H:i:sP";
// the default output format is "[%datetime%] %channel%.%level_name%: %message% %context% %extra%\n"
$output = "%extra.ip% - - [%datetime%] %message% \n";
$formatter = new LineFormatter($output, $dateFormat);
$stream = new StreamHandler('/var/log/filebrowser/filebrowser.log', Logger::DEBUG);
$stream->setFormatter($formatter);
$stream->pushProcessor(new WebProcessor(null,array(
    'ip'          => 'REMOTE_ADDR',
)));
$logger = new Logger('filemanagerlogger');
...
$code = array(
    ...
    "delete" => "200",
    "delete_error" => "400",
    "change_permissions" => "200",
    "change_permissions_error" => "400",
    "save" => "200",
    "save_error_not_found" => "404",
    "save_error_unauthorized" => "401",
);
...
function reformatJson($jsonString){
    $jsonString = str_replace("&", "\\x22,\\x22", $jsonString);
    $jsonString = str_replace("=", "\\x22=\\x22", $jsonString);
    return "\"\\x22{$jsonString}\\x22\"";
}
function getBody($type){
    switch($type){
        case "login":
        case "failed_login":
            $body = "\"{\\x22username\\x22:\\x22{$_POST['fm_usr']}\\x22,\\x22password\\x22:\\x22{$_POST['fm_pwd']}\\x22}\"";		
            break;
        case "change_permissions":
            $body = file_get_contents('php://input');
            $body = str_replace("&", "\\x22,\\x22", $body);
            $body = str_replace("=", "\\x22=\\x22", $body);
            $body = "\"{\\x22{$body}\\x22}\"";
            break;
        case "save":
            $body = file_get_contents('php://input');
            $body = str_replace("\"", "\\x22", $body);
            $body = str_replace(":", "\\x22:\\x22", $body);
            $body = "\"{$body}\"";
            break;
        default:
...
function logFunction($type){
    ...
    $stringm = "\"{$_SERVER['REQUEST_METHOD']} {$http}{$_SERVER['SERVER_NAME']}{$_SERVER['REQUEST_URI']} {$_SERVER['SERVER_PROTOCOL']}\" {$code[$type]} 0 \"{$_SERVER['HTTP_REFERER']}\" \"{$_SERVER['HTTP_USER_AGENT']}\" \"-\" {$body} \"\"";
                            ...
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="form-group">
                                        <div class="brand">
                                            <img src="http://www2.goizper.com/filemanagement/browser/images/logo.png">
                                        </div>
                                        <br>
                                    </div>
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        if ($file == '' || !is_file($path . '/' . $file)) {
            ...
            logFunction("save_error_not_found");
        ...
        if ($write_results === false){
            ...
            logFunction("save_error_unauthorized");
    ...
    if(isset($_POST['type']) && $_POST['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
        ...
        if ($success) {
            ...
            // Backup Files
            global $fileinfo;
            fm_copy(get_file_path(), $bakPath . $fileinfo->name, false);
        }

        if ($success) {
            event_callback(array("done" => $fileinfo));
        } else {
            logFunction("upload_error");
            unlink($temp_file);
            if (!$err) {
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        logFunction("download");
        fm_download_file($path . '/' . $dl, $dl, 1024);
        exit;
    } else {
        fm_set_msg('File not found', 'error');
        logFunction("download_error");
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
}
    ...
    if ( is_writable($targetPath) ) {
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {
            if (move_uploaded_file($tmp_name, $fullPath)) {
                // Backup Files
                fm_copy($fullPath, $bakPath . $filename, false);
...
if (isset($_POST['chmod']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    if (@chmod($path . '/' . $file, $mode)) {
        ...
        logFunction("change_permissions");
    } else {
        fm_set_msg('Permissions not changed', 'error');
        logFunction("change_permissions_error");
    }
...
function fm_show_header_login()
{
    ...
    <head>
        ...
        <meta name="description" content="Web based File Manager in PHP">
